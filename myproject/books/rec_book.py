...
def get_recommendations(book_title, genre, num_recommendations=4):
    ...
    if book_title not in df['Title_Lower'].values:
        ...
        recommended_books = dff.iloc[book_indices][['Title', 'Author', 'Description', 'Thumbnail','PreviewLink','AverageRating']].to_dict('records')
    ...
    book_indices = [i[0] for i in sim_scores]
    recommended_books = df.iloc[book_indices][['Title', 'Author', 'Description', 'Thumbnail','PreviewLink','AverageRating']].to_dict('records')
